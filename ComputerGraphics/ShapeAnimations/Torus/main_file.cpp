...
void initOpenGLProgram(GLFWwindow* window) {
	...
	glEnable(GL_DEPTH_TEST); //w części inicjującej ---------------------------------------------------------zmiana
...
void drawScene2(GLFWwindow* window, float angle) {
	...
	glm::mat4 M2 = glm::mat4(3.0f);
	M2 = glm::rotate(M2, angle, glm::vec3(0.0f, 1.0f, 0.0f));
	//Załadowanie macierzy modelu do programu cieniującego
	glUniformMatrix4fv(spLambert->u("M2"), 1, false, glm::value_ptr(M2));
	glUniform4f(spLambert->u("color"), 0, 0, 1, 1);
	Models::torus.drawSolid(); //Narysowanie obiektu
	//-----------
	glfwSwapBuffers(window); //Skopiowanie bufora ukrytego do widocznego. 
	
	////Co tu sie dzieje? czemu nie ma 2 figur
}
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window)) //Tak długo jak okno nie powinno zostać zamknięte
	{
		...
		glfwSetTime(0); //Wyzeruj timer
		drawScene2(window, angle); //Wykonaj procedurę rysującą
